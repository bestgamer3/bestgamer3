...
namespace
{
    struct Settings
    {
        ...
        bool enableViewmodel = true;
        WeaponViewmodelCalibration weaponCalibration{};
    };
    ...
    {
        Settings settings;
        bool shoulderOffsetSpecified = false;
        bool shoulderAnglesSpecified = false;
        ...
        for (int i = 1; i < argc; ++i)
        {
            ...
            try
            {
                ...
                else if (arg == L"--weapon-forward-cm" && i + 1 < argc)
                {
                    settings.weaponCalibration.hip.offsetMeters[0] =
                        std::clamp(std::stof(argv[++i]), -50.0f, 50.0f) / 100.0f;
                }
                else if (arg == L"--weapon-left-cm" && i + 1 < argc)
                {
                    settings.weaponCalibration.hip.offsetMeters[1] =
                        std::clamp(std::stof(argv[++i]), -50.0f, 50.0f) / 100.0f;
                }
                else if (arg == L"--weapon-up-cm" && i + 1 < argc)
                {
                    settings.weaponCalibration.hip.offsetMeters[2] =
                        std::clamp(std::stof(argv[++i]), -50.0f, 50.0f) / 100.0f;
                }
                else if (arg == L"--weapon-pitch" && i + 1 < argc)
                {
                    settings.weaponCalibration.hip.anglesDegrees[0] =
                        std::clamp(std::stof(argv[++i]), -90.0f, 90.0f);
                }
                else if (arg == L"--weapon-yaw" && i + 1 < argc)
                {
                    settings.weaponCalibration.hip.anglesDegrees[1] =
                        std::clamp(std::stof(argv[++i]), -90.0f, 90.0f);
                }
                else if (arg == L"--weapon-roll" && i + 1 < argc)
                {
                    settings.weaponCalibration.hip.anglesDegrees[2] =
                        std::clamp(std::stof(argv[++i]), -90.0f, 90.0f);
                }
                else if (arg == L"--shoulder-forward-cm" && i + 1 < argc)
                {
                    shoulderOffsetSpecified = true;
                    settings.weaponCalibration.shouldered.offsetMeters[0] =
                        std::clamp(std::stof(argv[++i]), -50.0f, 50.0f) / 100.0f;
                }
                else if (arg == L"--shoulder-left-cm" && i + 1 < argc)
                {
                    shoulderOffsetSpecified = true;
                    settings.weaponCalibration.shouldered.offsetMeters[1] =
                        std::clamp(std::stof(argv[++i]), -50.0f, 50.0f) / 100.0f;
                }
                else if (arg == L"--shoulder-up-cm" && i + 1 < argc)
                {
                    shoulderOffsetSpecified = true;
                    settings.weaponCalibration.shouldered.offsetMeters[2] =
                        std::clamp(std::stof(argv[++i]), -50.0f, 50.0f) / 100.0f;
                }
                else if (arg == L"--shoulder-pitch" && i + 1 < argc)
                {
                    shoulderAnglesSpecified = true;
                    settings.weaponCalibration.shouldered.anglesDegrees[0] =
                        std::clamp(std::stof(argv[++i]), -90.0f, 90.0f);
                }
                else if (arg == L"--shoulder-yaw" && i + 1 < argc)
                {
                    shoulderAnglesSpecified = true;
                    settings.weaponCalibration.shouldered.anglesDegrees[1] =
                        std::clamp(std::stof(argv[++i]), -90.0f, 90.0f);
                }
                else if (arg == L"--shoulder-roll" && i + 1 < argc)
                {
                    shoulderAnglesSpecified = true;
                    settings.weaponCalibration.shouldered.anglesDegrees[2] =
                        std::clamp(std::stof(argv[++i]), -90.0f, 90.0f);
                }
                ...
                else if (arg == L"--no-viewmodel")
                {
                    settings.enableViewmodel = false;
                }
            }
            catch (...)
            {
        ...
        // Unless a separate shouldered layer was requested, keep the calibrated
        // gun fit unchanged while ADS/two-hand shouldering blends in.
        if (!shoulderOffsetSpecified)
        {
            settings.weaponCalibration.shouldered.offsetMeters =
                settings.weaponCalibration.hip.offsetMeters;
        }
        if (!shoulderAnglesSpecified)
        {
            settings.weaponCalibration.shouldered.anglesDegrees =
                settings.weaponCalibration.hip.anglesDegrees;
        }
...
int wmain(int argc, wchar_t** argv)
{
    ...
    std::cout
        << "MW2 Campaign VR - Experimental Phase 3B Controller Weapon Core\n"
           "Single-player only: iw4sp.exe\n"
           "F5=viewmodel publish  F6=flip yaw  F7=flip lean X  F8=recenter\n"
           "F9=head-look  F10=6DoF  F12=quit companion\n"
        << "FOV=" << settings.fov
        << "  head gain=" << settings.mouseGain
        << "  controller gain=" << settings.controllerGain
        << "  world scale=" << settings.unitsPerMeter << " units/m\n\n"
           "Rift Touch mapping:\n"
           "  Right aim/grip       = physical weapon pose source\n"
           "  Left grip + squeeze = two-hand foregrip stabilization\n"
           "  Right trigger        = fire\n"
           "  Left trigger         = ADS / shouldered calibration blend\n"
           "  Left stick           = WASD movement\n"
           "  X                     = reload\n"
           "  A                     = jump\n\n"
           "Phase 3B publishes a COD4-style rigid weapon transform to the 32-bit\n"
           "MW2VRViewmodelHook.dll. The eye-centering renderer remains Phase 2B.\n\n";
    ...
    VRWeaponViewmodel weaponViewmodel;
    VRViewmodelBridge viewmodelBridge;
    const bool viewmodelBridgeAvailable = viewmodelBridge.Initialize();

    bool headLookEnabled = true;
    bool sixDofEnabled = settings.enable6Dof;
    bool viewmodelEnabled = settings.enableViewmodel;
    bool camera6DofAvailable = false;
    bool recenterRequested = true;

    // Keep the horizontal live calibration from Phase 3A. It is independent of
    // the physical weapon packet and remains useful across Oculus runtime builds.
    ...
    auto previousViewmodelFrame = std::chrono::steady_clock::now();
    HWND gameWindow = nullptr;

    bool previousSupportGrip = false;
    bool lastF5 = false;
    bool lastF6 = false;
    bool lastF7 = false;
    bool lastF8 = false;
    ...
    while (game.IsAlive())
    {
        ...
        if (f5 && !lastF5)
        {
            ...
            weaponViewmodel.Reset();
            if (!viewmodelEnabled)
            {
                viewmodelBridge.Clear();
            }
            std::cout
                << "Physical weapon viewmodel pose publishing "
                << (viewmodelEnabled ? "enabled" : "disabled") << ".\n";
        }
        if (f6 && !lastF6)
        {
            invertYaw = !invertYaw;
        ...
        const double viewmodelElapsed = std::chrono::duration<double>(
            now - previousViewmodelFrame).count();
        previousViewmodelFrame = now;

        const WeaponViewmodelPose physicalWeaponPose = weaponViewmodel.Update(
            *pose,
            touch,
            viewmodelElapsed,
            settings.weaponCalibration);

        if (viewmodelBridgeAvailable)
        {
            viewmodelBridge.Publish(
                physicalWeaponPose,
                settings.weaponCalibration,
                touch,
                settings.unitsPerMeter,
        ...
        if (physicalWeaponPose.supportGripActive != previousSupportGrip)
        {
            previousSupportGrip = physicalWeaponPose.supportGripActive;
            std::cout
                << "Viewmodel: support hand "
                << (previousSupportGrip ? "engaged" : "released") << ".\n";
        }
        ...
        // Keep the Phase 3A controller-to-mouse fallback active until the exact
        // stock iw4sp viewmodel placement call is detoured to the Win32 hook.
        // Once that detour is verified, this fallback can be disabled so weapon
        // aim becomes fully independent from head/camera orientation.
